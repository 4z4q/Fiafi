...
export default function AttractMode({ perfumes }: AttractModeProps) {
  ...
  useEffect(() => {
    ...
    slideTimer.current = setTimeout(() => {
      setCurrentIdx(
        (prev) => (prev + 1) % Math.max(shuffled.current.length, 1)
      );
    }, SLIDE_DURATION);
    return () => {
      if (slideTimer.current) clearTimeout(slideTimer.current);
  ...
  const heroGradient = `
    radial-gradient(ellipse at 60% 20%, ${
      perfume.main_accords[0]?.color_hex
    }35 0%, transparent 55%),
    radial-gradient(ellipse at 20% 80%, ${
      perfume.main_accords[1]?.color_hex || perfume.main_accords[0]?.color_hex
    }25 0%, transparent 55%),
      ...
      {isIdle && (
        ...
        >
          ...
          <div
            ...
            style={{
              backgroundColor: perfume.main_accords[1]?.color_hex || "#C8A03C",
            }}
          />
          ...
          {/* Slide indicator - عداد خفيف بدل ما نرندر عنصر DOM لكل عطر (بيوصل مئات) */}
          <div className="absolute bottom-8 left-1/2 -translate-x-1/2 z-20 flex items-center gap-3">
            <div className="h-0.5 w-32 rounded-full bg-[oklch(0.78_0.14_82_/_0.25)]">
              <div
                className="h-0.5 rounded-full bg-[oklch(0.78_0.14_82)] transition-all duration-500"
                style={{
                  width: `${
                    ((currentIdx + 1) / Math.max(shuffled.current.length, 1)) *
                    100
                  }%`,
                }}
              />
            </div>
            <span className="text-[10px] tracking-widest text-gold/70">
              {currentIdx + 1} / {shuffled.current.length}
            </span>
          </div>
