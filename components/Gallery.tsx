      ...
      <header className="sticky top-0 z-40 border-b border-border/70 bg-background/90 backdrop-blur-xl">
        <div className="mx-auto flex max-w-7xl items-center justify-between gap-6 px-5 py-4 lg:px-8">
          ...
          <nav className="hidden items-center gap-8 text-sm text-muted-foreground md:flex" aria-label="التنقل الرئيسي">
            ...
            <a className="transition-colors hover:text-primary" href="#story">حكاية فيافي</a>
      ...
      <main id="top">
        <section className="mx-auto grid max-w-7xl items-center gap-10 px-5 pb-20 pt-14 lg:grid-cols-[1fr_0.9fr] lg:px-8 lg:pb-28 lg:pt-24">
          <motion.div initial={{ opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7 }} className="order-2 lg:order-1">
            ...
            <div className="mt-9 flex flex-wrap items-center gap-4"><a href="#collection" className="inline-flex items-center gap-3 rounded-full bg-primary px-6 py-3 text-sm font-semibold text-primary-foreground transition-transform hover:-translate-y-0.5">تصفّح المجموعة <ArrowLeft data-icon="inline-start" /></a><a href="#story" className="text-sm text-foreground underline decoration-primary/40 underline-offset-8 hover:text-primary">اكتشف حكاية فيافي</a></div>
        ...
        <section id="story" className="border-y border-border bg-card"><div className="mx-auto grid max-w-7xl gap-8 px-5 py-16 lg:grid-cols-[0.7fr_1.3fr] lg:px-8 lg:py-24"><p className="text-sm font-semibold tracking-[0.25em] text-primary">حكاية فيافي</p><div><p className="max-w-3xl font-display text-3xl font-light leading-[1.6] text-pretty sm:text-4xl">نؤمن أن العطر ليس مجرد رائحة، بل <span className="font-semibold text-primary">مساحة من الذاكرة</span>، وتوقيع خفي يسبقك ويبقى بعدك.</p><p className="mt-6 max-w-xl text-sm leading-8 text-muted-foreground">من الرفوف المختارة إلى تفاصيل التجربة، نصنع رحلة هادئة للباحثين عن رائحة تحمل معنى.</p></div></div></section>

        <section className="mx-auto max-w-7xl px-5 py-16 lg:px-8 lg:py-24"><div className="mb-8 flex items-end justify-between"><div><p className="text-xs tracking-[0.22em] text-primary">اختر مزاجك</p><h2 className="mt-3 font-display text-3xl font-semibold">عالم من الروائح</h2></div><Sparkles className="text-primary/40" /></div><div className="grid gap-px overflow-hidden rounded-2xl border border-border bg-border sm:grid-cols-3">{categories.map((category, index) => <a key={category.name} href="#collection" onClick={() => setSelectedGender(category.name)} className="group bg-background p-7 transition-colors hover:bg-secondary"><span className="text-xs text-muted-foreground">0{index + 1}</span><h3 className="mt-10 font-display text-2xl font-semibold group-hover:text-primary">{category.name}</h3><p className="mt-3 text-xs text-muted-foreground">{category.count} عطر مختار <ArrowUpLeft className="inline transition-transform group-hover:-translate-x-1 group-hover:-translate-y-1" /></p></a>)}</div></section>
        ...
        <section id="brands" className="mx-auto max-w-7xl px-5 py-16 lg:px-8 lg:py-24"><div className="flex flex-col gap-8 sm:flex-row sm:items-center sm:justify-between"><div><p className="text-xs tracking-[0.22em] text-primary">OUR WORLD</p><h2 className="mt-3 font-display text-3xl font-semibold">أسماء لها حضور</h2></div><div className="grid flex-1 gap-x-8 gap-y-4 sm:max-w-2xl sm:grid-cols-3">{brands.slice(1, 7).map((brand) => <button key={brand} onClick={() => { setSelectedBrand(brand); document.getElementById('collection')?.scrollIntoView({ behavior: 'smooth' }) }} className="border-b border-border pb-3 text-right text-sm text-muted-foreground transition hover:border-primary hover:text-primary">{brand}</button>)}</div></div></section>
